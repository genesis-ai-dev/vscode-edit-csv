...
export function activate(context: vscode.ExtensionContext) {
  ...
  const applyCsvCommand = vscode.commands.registerCommand(
    "edit-csv.apply",
    () => {
  ...
  const applyAndSaveCsvCommand = vscode.commands.registerCommand(
    "edit-csv.applyAndSave",
    () => {
      const instance = getActiveEditorInstance(instanceManager);
      if (!instance) return;
  ...
  const gotoSourceCsvCommand = vscode.commands.registerCommand(
    "edit-csv.goto-source",
    () => {
      if (vscode.window.activeTextEditor) {
        //a web view is no text editor...
        vscode.window.showInformationMessage(
          "Open a csv editor first to show the source csv file"
        );
        return;
      }
  ...
  const editCsvCommand = vscode.commands.registerCommand(
    "edit-csv.edit",
    () => {
  ...
  const editCsvWithConfigCommand = vscode.commands.registerCommand(
    "edit-csv.editWithConfig",
    (overwriteConfigObj: EditCsvConfigOverwrite | undefined) => {
  ...
  const onDidOpenTextDocumentHandler = vscode.workspace.onDidOpenTextDocument(
    (args) => {
      ...
      if (args.isUntitled || isCsvFile(args) === false || args.version !== 1)
        return;
  ...
  // })
  const onDidChangeConfigurationCallback = onDidChangeConfiguration.bind(
    undefined,
    instanceManager
  );
...
function onDidChangeConfiguration(
  ...
  e: vscode.ConfigurationChangeEvent | null
) {
...
function openSourceFileFunc(instanceManager: InstanceManager): void {
  ...
  try {
    ...
    vscode.window.showErrorMessage(
      `Could not find the source file for the editor (no instance found), error: ${error.message}`
    );
    return;
  }
...
function beforeEditCsvCheck(instanceManager: InstanceManager): boolean {
  if (
    !vscode.window.activeTextEditor &&
    instanceManager.hasActiveEditorInstance()
  ) {
  ...
  if (
    ...
    vscode.window.showInformationMessage(
      "Open a csv file first to show the csv editor or file too large (> 50MB)"
    );
    return false;
  }
...
function createNewEditorInstance(
  ...
  let isInCurrentWorkspace =
    ...
    vscode.workspace.asRelativePath(activeTextEditor.document.uri.fsPath);
  ...
  if (isInCurrentWorkspace) {
    ...
    if (config.shouldWatchCsvSourceFile !== "no") {
      ...
      watcher = vscode.workspace.createFileSystemWatcher(
        new vscode.RelativePattern(activeTextEditor.document.fileName, "*"),
        true,
        false,
        true
      );
  ...
  if (
    activeTextEditor.document.lineAt(activeTextEditor.selection.active.line)
      .text.length === activeCol
  ) {
    activeCol =
      activeTextEditor.document.lineAt(activeTextEditor.selection.active.line)
        .text.length - 1;
  ...
  panel.webview.html = createEditorHtml(panel.webview, context, config, {
    ...
    isCursorPosAfterLastColumn:
      activeTextEditor.document.lineAt(activeTextEditor.selection.active.line)
        .text.length === activeTextEditor.selection.active.character,
...
function applyContent(
  ...
  vscode.workspace.openTextDocument(instance.sourceUri).then(
    ...
    (reason) => {
      ...
      console.warn(
        `Could not find the source file, trying to access it and create a temp file with the same path...`
      );
      console.warn(reason);

      vscode.workspace.fs.stat(instance.sourceUri).then(
        (fileStat) => {
          //file exists and can be accessed
          vscode.window.showErrorMessage(
            `Could apply changed because the source file could not be found`
          );
        },
        (error) => {
...
function createNewSourceFile(
  ...
  const newSourceFile = vscode.Uri.parse(
    `untitled:${instance.sourceUri.fsPath}`
  );

  vscode.workspace.openTextDocument(newSourceFile).then(
    (newFile) => {
      const edit = new vscode.WorkspaceEdit();
      ...
      vscode.workspace.applyEdit(edit).then(
        (success) => {
          if (!success) {
            debugLog(
              "could not created new source file because old was deleted"
            );
            return;
          }
          ...
          if (saveSourceFile) {
            newFile.save().then(
              (successSave) => {
                if (!successSave) {
                  vscode.window.showErrorMessage(
                    `Could not save new source file (old was deleted)`
                  );
                  return;
                }
              ...
              (error) => {
                vscode.window.showErrorMessage(
                  `Could not save new source file (old was deleted), error: ${error?.message}`
                );
              }
            );
          }
        ...
        (error) => {
          vscode.window.showErrorMessage(
            `Could not create new source file (old was deleted), error: ${error?.message}`
          );
        }
      );
    },
    (error) => {
      vscode.window.showErrorMessage(
        `Could not open new source file, error: ${error?.message}`
      );
    }
  );
}
...
function getActiveEditorInstance(
  ...
  if (vscode.window.activeTextEditor) {
    //a web view is no text editor...
    vscode.window.showInformationMessage(
      "Open a csv editor first to apply changes"
    );
    return null;
  }
  ...
  try {
    ...
    vscode.window.showErrorMessage(
      `Could not find the editor instance, error: ${error.message}`
    );
    return null;
  }
...
function onWatcherChangeDetecedHandler(
  e: vscode.Uri,
  instanceManager: InstanceManager
) {
