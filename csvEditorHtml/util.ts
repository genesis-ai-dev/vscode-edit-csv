...
function isCommentCell(value: string | null, csvReadConfig: CsvReadOptions) {
  ...
  if (
    typeof csvReadConfig.comments === "string" &&
    csvReadConfig.comments !== ""
  ) {
...
function createCellValueWithUrlLinks(
  text: string,
  urls: UrlInStringCoords[]
): (string | HTMLElement)[] {
 ...
 * @param csvReadConfig
 */
function _normalizeDataArray(
  csvParseResult: ExtendedCsvParseResult,
  csvReadConfig: CsvReadOptions,
  fillString = ""
) {
  const maxCols = csvParseResult.data.reduce(
  ...
  for (let i = 0; i < csvParseResult.data.length; i++) {
    ...
    if (
      isCommentCell(row[0], csvReadConfig) === false &&
      firstRealRowExpandedWasFound === false
    ) {
      ...
      if (row.length < maxCols && csvParseResult.columnIsQuoted !== null) {
        csvParseResult.columnIsQuoted.push(
          ...Array.from(
            Array(maxCols - row.length),
            (p, index) => newColumnQuoteInformationIsQuoted
          )
        );
      }
    }
    ...
    if (row.length < maxCols) {
      row.push(
        ...Array.from(Array(maxCols - row.length), (p, index) => fillString)
      );
...
function _resolveInitiallyHiddenColumns(
  csvParseResult: ExtendedCsvParseResult,
  csvReadConfig: CsvReadOptions
) {
  ...
  let _initiallyHiddenColumnNames =
    initialConfig.initiallyHiddenColumnNames ?? [];
  let _initiallyHiddenColumnNumbers =
    initialConfig.initiallyHiddenColumnNumbers ?? [];

  if (
    _initiallyHiddenColumnNames.length === 0 &&
    _initiallyHiddenColumnNumbers.length === 0
  )
    return;
  ...
  for (let i = 0; i < _initiallyHiddenColumnNames.length; i++) {
    ...
    if (indicesToHide) {
      indicesToHide.forEach((index) =>
        initiallyHiddenColumnIndicesSet.add(index)
      );
    }
  }
  ...
  if (initiallyHiddenColumnIndices.length === firstRealRow.length) {
    ...
    console.warn(
      `initially hidden columns: all columns are hidden, this is not allowed`
    );
    initiallyHiddenColumnIndices = [];
  }
...
function _unhideAllColumns() {
  ...
  for (let i = 0; i < hiddenPhysicalColumnIndicesSorted.length; i++) {
    const visualColIndex = hot.toVisualColumn(
      hiddenPhysicalColumnIndicesSorted[i]
    );
...
function _getCommentIndices(
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (
      row.length > 0 &&
      row[0] !== null &&
      isCommentCell(row[0], csvReadConfig)
    ) {
...
function _insertRowInternal(belowCurrRow: boolean) {
  ...
  const focusBehavior =
    initialConfig?.insertRowBehavior ?? "focusFirstCellNewRow";
  ...
  switch (focusBehavior) {
    ...
    default:
      notExhaustiveSwitch(focusBehavior);
  }
...
function pretendRemoveRowContextMenuActionClicked() {
  ...
  // Show confirmation instead of direct deletion
  pendingRowDeleteIndex = currRowIndex;
  toggleDeleteRowModal(true);
}
 ...
 */
function commentValueAndUrlsRenderer(
  ...
  if (highlightCsvComments) {
    if (
      value !== null &&
      col === 0 &&
      isCommentCell(value, defaultCsvReadOptions)
    ) {
      // td.classList.add('comment-row')
      if (td && td.nextSibling) {
        (td.nextSibling as HTMLElement).title =
          warningTooltipTextWhenCommentRowNotFirstCellIsUsed;
      }
...
(Handsontable.renderers as any).registerRenderer(
  "commentValueAndUrlsRenderer",
  commentValueAndUrlsRenderer
);
 ...
 */
function _setOption<T extends {}>(
  targetOptions: T,
  options: T,
  optionName: keyof T & string
) {
...
function setCsvReadOptionsInitial(options: CsvReadOptions) {
  ...
  el4.value =
    defaultCsvReadOptions.comments === false
      ? ""
      : defaultCsvReadOptions.comments;
...
function setCsvWriteOptionsInitial(options: CsvWriteOptions) {
  ...
  el3.value =
    defaultCsvWriteOptions.comments === false
      ? ""
      : defaultCsvWriteOptions.comments;
...
function checkIfHasHeaderReadOptionIsAvailable(
  ...
  if (canSetOption) {
    ...
    if (firstRow === null && !el.checked) {
      //if el.checked is true then we already have a header row...
      canSetOption = false;
    }
  }
 ...
 */
function setupAndApplyInitialConfigPart1(
  initialConfig: EditCsvConfig | undefined,
  ...
  newColumnQuoteInformationIsQuoted =
    initialConfig.newColumnQuoteInformationIsQuoted;
  ...
  let _readOption_hasHeader =
    initialConfig.readOption_hasHeader === "true" ? true : false;
  ...
  switch (initialConfig.optionsBarAppearance) {
    ...
    default: {
      _error(
        `unknown optionsBarAppearance: ${initialConfig.optionsBarAppearance}`
      );
  ...
  for (let i = 0; i < document.styleSheets.length; i++) {
    ...
    if (
      !styleSheet.href?.toLowerCase().endsWith(`dark.css`) &&
      ...
      if (cssRule instanceof CSSStyleRule) {
        ...
        if (cssStyleRule.selectorText === `body.vscode-light`) {
          cssStyleRule.style.setProperty(
            cssFgColorVariableName,
            initialConfig.lightThemeTextColor
          );
        }
        ...
        if (cssStyleRule.selectorText === `body.vscode-dark`) {
          cssStyleRule.style.setProperty(
            cssFgColorVariableName,
            initialConfig.darkThemeTextColor
          );
        }
      }
    }
    ...
    for (let i = minC; i <= maxC; i++) {
      ...
      if (el !== "" && el !== null) {
        ...
        if (!containsInvalidNumbers) {
          const firstCanonicalNumberStringInCell =
            getFirstCanonicalNumberStringInCell(el, numbersStyleToUse);
          ...
          try {
            ...
            console.warn(
              `could not create or add number to statSumOfNumbers at row: ${index}, col: ${i}`,
              error
            );
 ...
 */
function getFirstCanonicalNumberStringInCell(
  cellValue: string,
  numbersStyle: NumbersStyle
): string | null {
  ...
  while (
    (thousandSeparatorsMatches =
      numbersStyle.thousandSeparatorReplaceRegex.exec(cellValue))
  ) {
    let replaceContent = thousandSeparatorsMatches[0].replace(
      numbersStyle.thousandSeparator,
      ""
    );
    cellContent = cellContent.replace(
      thousandSeparatorsMatches[0],
      replaceContent
    );
  }
 ...
 */
function setNumbersStyleUi(
  numbersStyleToUse: EditCsvConfig["initialNumbersStyle"]
) {
...
const b = new Big(1);
function formatBigJsNumber(
  bigJsNumber: typeof b,
  numbersStyleToUse: NumbersStyle
): string {
...
function calcHotCellToSelectFromCurosPos(
  ...
  try {
    ...
    if (
      sourceFileCursorLineIndex <
      csvParseResult.outLineIndexToCsvLineIndexMapping.length
    ) {
      ...
      currentCsvRowStartTextFileLineIndex =
        csvParseResult.outLineIndexToCsvLineIndexMapping.indexOf(
          csvRowToSelect
        );
      ...
      if (currentCsvRowStartTextFileLineIndex === -1) {
        throw new Error(
          `Could not find text line for csv row ${csvRowToSelect}`
        );
      }
      ...
      {
        for (
          let i = currentCsvRowStartTextFileLineIndex;
          i < csvParseResult.outLineIndexToCsvLineIndexMapping.length;
          i++
        ) {
      ...
      if (
        currentCsvRowStartTextFileLineIndex !==
        currentCsvRowEndTextFileLineIndex
      ) {
    ...
    if (
      openTableAndSelectCellAtCursorPos === "initialOnly_correctRowAndColumn"
    ) {
      //initial only is row AND col position

      let fieldEndIndices =
        csvParseResult.outColumnIndexToCsvColumnIndexMapping[csvRowToSelect];
    ...
    if (csvReadOptions._hasHeader) {
      ...
      for (let i = 0; i < csvParseResult.data.length; i++) {
        ...
        if (
          csvRow.length > 0 &&
          isCommentCell(csvRow[0], csvReadOptions) === false
        ) {
          csvHeaderRowIndex = i;
          break;
        }
    ...
    console.warn(
      `[edit csv] could not select line ${
        sourceFileCursorLineIndex ?? -1
      } or column ${sourceFileCursorColumnIndex ?? -1}`,
      e
    );
    return {
      rowIndex: 0,
  ...
  selectCellPos.rowIndex = getNextRowIfCommentsAreHidden(
    selectCellPos.rowIndex
  );
  selectCellPos.rowIndex += modifySelectedRowBecauseIfHeaderBy;

  try {
    selectCellPos.rowIndex = Math.max(
      0,
      Math.min(csvParseResult.data.length - 1, selectCellPos.rowIndex)
    );
    let dataRow = csvParseResult.data[selectCellPos.rowIndex];
    selectCellPos.colIndex = Math.max(
      0,
      Math.min(dataRow.length - 1, selectCellPos.colIndex)
    );
  } catch (e) {
...
function scrollToSelectedCell(hot: Handsontable, cellToSelect: HotCellPos) {
  ...
  if (cellToSelect.rowIndex !== 0) {
    ...
    viewportTopRowToShow =
      cellToSelect.rowIndex - Math.floor(visibleRowCount / 2);
    ...
    viewportTopRowToShow = Math.max(
      0,
      Math.min(viewportTopRowToShow, maxRowCount - 1)
    );
  }
  ...
  if (cellToSelect.colIndex !== 0) {
    ...
    for (let i = 0; i <= lastVisibleCol; i++) {
      ...
      if (i === 0) {
        ...
        cumulativeAbsoluteOffsets.push(
          colWidths[i] + cumulativeAbsoluteOffsets[i - 1]
        );
      }
    }
    ...
    if (
      firstVisibleCol === 0 &&
      selectedCellLeftOffsetMiddle <= viewportWidth / 2
    ) {
      ...
      let viewportAbsoluteMiddle =
        viewportAbsoluteLeftOffset + Math.floor(viewportWidth / 2);
      let viewportMiddleDiff =
        selectedCellLeftOffsetMiddle - viewportAbsoluteMiddle;
      //the absolute left offset needed to center the column in the viewport
      let neededViewporLeftOffsetAbsolute =
        viewportAbsoluteLeftOffset + viewportMiddleDiff;
...
function setHotScrollPosition(
  hot: Handsontable,
  viewportOffsetInPx: HotViewportOffsetInPx
): void {
  (hot as any).view.activeWt.wtOverlays.topOverlay.setScrollPosition(
    viewportOffsetInPx.top
  );
  (hot as any).view.activeWt.wtOverlays.leftOverlay.setScrollPosition(
    viewportOffsetInPx.left
  );
}
...
function getNextRowIfCommentsAreHidden(visualRowIndex: number): number {
  ...
  for (let i = visualRowIndex; i <= lastPossibleRowIndex; i++) {
    ...
    let isRowCommentAndHidden =
      hiddenPhysicalRowIndicesSorted.indexOf(physicalIndex) !== -1;
...
function getFirstAndLastVisibleColumns(): { first: number; last: number } {
  ...
  for (let index = 0; index <= lastPossibleColIndex; index++) {
    ...
    let isColHidden =
      hiddenPhysicalColumnIndicesSorted.indexOf(physicalColIndex) !== -1;
...
function getFirstAndLastVisibleRows(): { first: number; last: number } {
  ...
  for (let index = 0; index <= lastPossibleRowIndex; index++) {
    ...
    let isRowHidden =
      hiddenPhysicalRowIndicesSorted.indexOf(physicalRowIndex) !== -1;
...
function confirmDeleteRow() {
  console.log("confirmDeleteRow", { pendingRowDeleteIndex, hot });
  if (pendingRowDeleteIndex !== null) {
    // Actually perform the row deletion

    if (!hot) throw new Error("table was null");
    hot.alter("remove_row", pendingRowDeleteIndex);
    checkIfHasHeaderReadOptionIsAvailable(false);
