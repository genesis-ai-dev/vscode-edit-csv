...
if (typeof initialConfig === "undefined") {
  ...
  if (typeof initialConfig.doubleClickColumnHandleForcedWith === "string") {
    initialConfig.doubleClickColumnHandleForcedWith = parseInt(
      initialConfig.doubleClickColumnHandleForcedWith
    );
...
const regression: typeof import("../thirdParty/regression/regression").default =
  (window as any).regression;
...
let _onTableScrollThrottled: ((this: HTMLDivElement, e: Event) => void) | null =
  null;
...
const btnApplyChangesToFileAndSave = _getById(
  `btn-apply-changes-to-file-and-save`
);

const readDelimiterTooltip = _getById("read-delimiter-tooltip");
const readDelimiterTooltipText = "Empty to auto detect";

const receivedCsvProgBar = _getById(
  "received-csv-prog-bar"
) as HTMLProgressElement;
const receivedCsvProgBarWrapper = _getById(
...
const fixedColumnsTopInfoSpan = _getById(
  "fixed-columns-top-info"
) as HTMLDivElement;
...
const newlineSameSsInputOption = _getById(
  "newline-same-as-input-option"
...
if (vscode && !leftSidePanelToggle)
  throw new Error(`element with id 'left-panel-toggle' not found`); //null is ok for browser
const leftPanelToggleIconExpand = document.getElementById(
  `left-panel-toggle-icon-expand`
) as HTMLElement; //<i>
if (vscode && !leftPanelToggleIconExpand)
  throw new Error(`element with id 'left-panel-toggle-icon-expand' not found`); //null is ok for browser
const sideBarResizeHandle = _getById(
...
const statSelectedCellsCount = _getById(
  `stat-selected-cells-count`
) as HTMLDivElement;
const statSelectedNotEmptyCells = _getById(
  `stat-selected-not-empty-cells`
) as HTMLDivElement;
...
const numbersStyleNonEnRadio = _getById(
  `numbers-style-non-en`
) as HTMLInputElement; //radio

const isReadonlyModeToggleSpan = _getById(
  `is-readonly-mode-toggle`
) as HTMLSpanElement;
