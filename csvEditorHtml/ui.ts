type ContextMenuSettings =
  import("../thirdParty/handsontable/handsontable").contextMenu.Settings;

type GridSettings =
  import("../thirdParty/handsontable/handsontable").GridSettings;
...
function toggleOptionsBar(shouldCollapse?: boolean) {
  ...
  document.documentElement.style.setProperty(
    "--extension-options-bar-display",
    shouldCollapse ? `none` : `block`
  );
...
function forceAutoResizeRows() {
  ...
  plugin.manualRowHeights.forEach(
    (height: number, physicalRowIndex: number) => {
...
function displayData(
  this: any,
  csvParseResult: ExtendedCsvParseResult | null,
  csvReadConfig: CsvReadOptions
) {
  ...
  const initiallyHideComments = initialConfig
    ? initialConfig.initiallyHideComments
    : false;

  if (initiallyHideComments && typeof csvReadConfig.comments === "string") {
    hiddenPhysicalRowIndicesSorted = _getCommentIndices(
      csvParseResult.data,
      csvReadConfig
    );
  ...
  const showColumnHeaderNamesWithLettersLikeExcel =
    initialConfig?.showColumnHeaderNamesWithLettersLikeExcel ?? false;

  let defaultColHeaderFuncBound = defaultColHeaderFunc.bind(
    this,
    showColumnHeaderNamesWithLettersLikeExcel
  );
  ...
  hot = new Handsontable(container, {
    ...
    rowHeaders: function (row: number) {
      ...
      let showDeleteRowHeaderButton =
        initialConfig?.showDeleteRowHeaderButton ?? true; //default is true

      if (
        csvParseResult.data.length === 1 ||
        isReadonlyMode ||
        showDeleteRowHeaderButton === false
      ) {
    ...
    fillHandle:
      initialConfig?.dragToAutoFill !== "none"
        ? {
            autoInsertRow: true,
          }
        : undefined,
    ...
    comments: false,
    autoWrapRow:
      initialConfig?.lastColumnOrFirstColumnNavigationBehavior === "stop"
        ? false
        : true, //seems wrong, but this way around is correct...
    autoWrapCol:
      initialConfig?.lastRowOrFirstRowNavigationBehavior === "stop"
        ? false
        : true, //seems wrong, but this way around is correct...
    search: {
      ...
      items: {
        ...
        remove_col: {
          disabled: function () {
            ...
            if (selection) {
              const selectedColsCount = Math.abs(
        ...
        hide_columns: {
          ...
          callback: function (
            ...
            for (
              let targetCol = _selection.start.col;
              targetCol <= _selection.end.col;
              targetCol++
            ) {
        ...
        edit_header_cell: {
          ...
          hidden: function () {
            ...
            if (
              selectedRanges[0][0] !== 0 ||
              selectedRanges[0][2] !== maxRowIndex
            )
              return true;
        ...
        resize_row_header_cell: {
          name: `Resize row to ${
            initialConfig?.doubleClickRowHandleForcedHeight ?? 106
          }px`,
          callback: function (
            key: string,
            selection: Array<{
        ...
        resize_column_header_cell: {
          ...
          callback: function (
            ...
            //also allow resizing multiple cols at once
            for (
              let i = selection[0].start.col;
              i <= selection[0].end.col;
              i++
            ) {
              // let colWidth = hot!.getColWidth(i)
              // allColWidths[i] = desiredColWidth
    ...
    beforeColumnResize: function (oldSize, newSize, isDoubleClick) {
      //after change but before render
    ...
    tabMoves: function (event: KeyboardEvent) {
      ...
      if (
        !initialConfig ||
        initialConfig.lastColumnTabBehavior !== "createColumn"
      )
        return _default;
      ...
      const isLastColSelected =
        selected[1] === firstAndLastVisibleColumns?.last;
    ...
    beforeUndo: function (
      _action:
        | EditHeaderCellAction
        | RemoveColumnAction
        | InsertColumnAction
        | any
    ) {
      let __action = _action as
        | EditHeaderCellAction
        | RemoveColumnAction
        | InsertColumnAction;
      ...
      if (__action.actionType === "changeHeaderCell" && headerRowWithIndex) {
        ...
        if (lastAction && lastAction.action === "removed") {
          headerRowWithIndex.row.splice(
            lastAction.visualIndex,
            0,
            ...lastAction.headerData
          );
    ...
    afterColumnMove: function (
      startColVisualIndices: number[],
      endColVisualIndex: number
    ) {
      if (!hot) throw new Error("table was null");
      ...
      if (headerRowWithIndex !== null) {
        ...
        const headerRowTexts: (string | null)[] = startColVisualIndices.map(
          (p) => temp.row[p]
        ); //for some reason we need tmp here else can be null??

        let headerRowCopy: Array<string | null> = [];

        for (let i = 0; i <= headerRowWithIndex.row.length; i++) {
          const colText =
            i < headerRowWithIndex.row.length
              ? headerRowWithIndex.row[i]
              : null;
      ...
      if (columnIsQuoted) {
        ...
        const startQuoteInformation: boolean[] = startColVisualIndices.map(
          (p) => columnIsQuoted[p]
        );
        ...
        for (let i = 0; i <= columnIsQuoted.length; i++) {
          const quoteInfo =
            i < columnIsQuoted.length ? columnIsQuoted[i] : false;
    ...
    afterCreateCol: function (visualColIndex, amount, source?: string) {
      ...
      if (columnIsQuoted) {
        // const physicalIndex = hot.toPhysicalColumn(visualColIndex)
        columnIsQuoted.splice(
          visualColIndex,
          0,
          ...Array(amount).fill(newColumnQuoteInformationIsQuoted)
        );
      }
    ...
    afterRemoveCol: function (
      ...
      let isFromUndoRedo =
        source === `UndoRedo.undo` || source === `UndoRedo.redo`;
    ...
    beforeSetRangeStart: function (nextCoords) {
      ...
      if (
        hiddenPhysicalRowIndicesSorted.length === 0 &&
        hiddenPhysicalColumnIndicesSorted.length === 0
      )
        return;
      ...
      const isFirstRowHidden =
        hiddenPhysicalRowIndicesSorted.indexOf(hot.toPhysicalRow(0)) !== -1;
      const isLastRowHidden =
        hiddenPhysicalRowIndicesSorted.indexOf(
          hot.toPhysicalRow(lastPossibleRowIndex)
        ) !== -1;

      const isFirstColHidden =
        hiddenPhysicalColumnIndicesSorted.indexOf(hot.toPhysicalColumn(0)) !==
        -1;
      const isLastColHidden =
        hiddenPhysicalColumnIndicesSorted.indexOf(
          hot.toPhysicalColumn(lastPossibleColIndex)
        ) !== -1;
      ...
      const wrapNavigationAfterFirstOrLastRow =
        initialConfig?.lastRowOrFirstRowNavigationBehavior === "wrap"
          ? true
          : false;
      const wrapNavigationAfterFirstOrLastCol =
        initialConfig?.lastColumnOrFirstColumnNavigationBehavior === "wrap"
          ? true
          : false;
      ...
      if (directionRow !== 0 && directionCol !== 0) {
        ...
        if (
          wrapNavigationAfterFirstOrLastCol === false &&
          ((directionRow === -1 &&
            nextCoords.col === firstAndLastVisibleColumns?.first) ||
            (directionRow === 1 &&
              nextCoords.col === firstAndLastVisibleColumns?.last))
        ) {
          //do not wrap
        } else {
        ...
        if (
          wrapNavigationAfterFirstOrLastRow === false &&
          ((directionCol === -1 &&
            nextCoords.row === firstAndLastVisibleRows?.first) ||
            (directionCol === 1 &&
              nextCoords.row === firstAndLastVisibleRows?.last))
        ) {
          //do not wrap
        } else {
    ...
    beforeKeyDown: function (event: KeyboardEvent) {
      ...
      if (
        ...
        event.key === "a" &&
        findWidgetInstance.isFindWidgetDisplayed()
      ) {
        event.stopImmediatePropagation();
      ...
      if (
        (!isMacOS &&
          event.altKey && //mac uses `ctrl+x` for focusing editors
          (event.key === `1` ||
            event.key === `2` ||
            event.key === `3` ||
            event.key === `4` ||
    ...
    beforeOnCellMouseDown: function (
      ...
      _blockCalculations: any
    ) {
      //if we want to open a link, prevent cell selection (only if we click on the link element)
      if (
        isOpenLinkModifierPressed(event) &&
        event.target &&
        (event.target as HTMLAnchorElement).tagName.toLowerCase() === "a"
      ) {
        let attrValue = (event.target as HTMLAnchorElement).getAttribute(
          linkIsOpenableAttribute
        );
...

  {
    ...
    autoColumnSizePlugin.ignoreCellWidthFunc = (value: string) => {
      const ignoreCommentCellWidths =
        initialConfig?.autoColumnWidthsIgnoreComments ?? true;
      ...
      return (
        (ignoreCommentCellWidths || commentsAreHidden) &&
        isCommentCell(value, csvReadConfig)
      );
    };
  }
  ...
  if (previousManualRowHeights) {
    ...
    //foreach skips over empty indices
    //manualRowHeights already stores physicalRow indices...
    previousManualRowHeights.forEach(
      (height: number, physicalRowIndex: number) => {
  ...
  copyPastePlugin.pasteScrollBehavior =
    initialConfig?.pasteScrollBehavior || `scrollToLastPastedCell`;
  ...
  if (oldShouldApplyHeaderReadOption === true) {
    if (settingsApplied === true) {
      //this must be applied else we get duplicate first row
  ...
  if (hot) {
    ...
    if (previousSelectedCell === null || previousViewportOffsets === null) {
      ...
      hot.selectCell(
        previousSelectedCell.rowIndex,
        previousSelectedCell.colIndex
      );
 ...
 */
function defaultColHeaderFunc(
  useLettersAsColumnNames: boolean,
  colIndex: number,
  colName: string | undefined | null
) {
  ...
  let showDeleteColumnHeaderButton =
    initialConfig?.showDeleteColumnHeaderButton ?? true; //default is true

  if (
    hot.countCols() === 1 ||
    isReadonlyMode ||
    showDeleteColumnHeaderButton === false
  ) {
...
function getRowHeaderWidth(rows: number) {
  ...
  const len =
    rows.toString().length * widthMultiplyFactor +
    binIcon +
    iconPadding +
    parentPadding +
    hiddenRowIcon;
...
function trimAllCells() {
  ...
  let changeSet: Array<
    [visualRowIndex: number, visualColIndex: number, value: string]
  > = [];
...
function showOrHideAllComments(show: boolean) {
  if (show) {
    ...
    if (hot) {
      ...
      hiddenPhysicalRowIndicesSorted = hiddenPhysicalRowIndicesSorted.map((p) =>
        hot!.toPhysicalRow(p)
      );
      hiddenPhysicalRowIndicesSorted = hiddenPhysicalRowIndicesSorted.sort();
    }
...
function changeFontSizeInPx(fontSizeInPx: number) {
  ...
  if (fontSizeInPx <= 0) {
    ...
    let fontSizeInPx = document.documentElement.style.getPropertyValue(
...
function setupSideBarResizeHandle() {
  ...
  document.addEventListener(
    ...
    throttle((e: MouseEvent) => {
      ...
      sidePanel.style.width = `${Math.max(
        downWidth + delta,
        minSidebarWidthInPx
      )}px`;
      sidePanel.style.maxWidth = `${Math.max(
        downWidth + delta,
        minSidebarWidthInPx
      )}px`;
...
function getHandsontableOverlayScrollLeft(): HTMLDivElement | null {
  const overlayWrapper = document.querySelector(
    `#csv-editor-wrapper .ht_master .wtHolder`
  );
...
function _onTableScroll(e: Event) {
  ...
  editHeaderCellTextInputEl.style.left = `${
    editHeaderCellTextInputLeftOffsetInPx -
    (scrollLeft - handsontableOverlayScrollLeft)
  }px`;
}
...
function getIsSidePanelCollapsed(): boolean {
  ...
  if (vscode) {
    return (
      window.getComputedStyle(leftPanelToggleIconExpand).display === "block"
    );
  }
...
function toggleSidePanel(shouldCollapse?: boolean) {
  ...
  document.documentElement.style.setProperty(
    "--extension-side-panel-display",
    shouldCollapse ? `none` : `flex`
  );

  document.documentElement.style.setProperty(
    "--extension-side-panel-expand-icon-display",
    shouldCollapse ? `block` : `none`
  );

  document.documentElement.style.setProperty(
    "--extension-side-panel-collapse-icon-display",
    shouldCollapse ? `none` : `block`
  );
...
function pre_afterRemoveCol(
  this: any,
  visualColIndex: number,
  amount: number,
  isFromUndoRedo: boolean
) {
...
function afterRemoveCol(
  ...
  const sortedColumnIds = sortConfigs.map((p) =>
    hot!.toPhysicalColumn(p.column)
  );
...
function _updateHandsontableSettings(
  settings: Handsontable.DefaultSettings,
  init: boolean,
  skipEnablingAutoColumnSizePlugin: boolean
) {
  if (!hot) return;
...
function __changeTableContentZoom(newScalerFinal: number) {
  ...
  document.documentElement.style.setProperty(
    "--hot-font-size-add-modifier",
    `${fontSizeAddModifier}px`
  );
